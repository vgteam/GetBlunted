/**
 * \file GaloisLattice.cpp
 *
 * Implements a representation of the Galois lattice of a domino-free bipartite graph.
 */
...
vector<bipartition> GaloisLattice::biclique_separator() const {
    ...
    for (size_t i : separator()) {
        return_val.emplace_back(galois_trees[bicliques[i].first].biclique(bicliques[i].second));
    }
    return return_val;
}
...
vector<size_t> GaloisLattice::separator() const {
    ...
    for (size_t i = 0, edge_idx = 0, cut_idx = 0; cut_idx < cut_edges.size(); ++i) {
        auto& edges = menger_graph[i];
