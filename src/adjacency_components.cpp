    ...
    graph.for_each_handle([&](const handle_t& handle) {
        for (handle_t side : {handle, graph.flip(handle)}) {
            if (handles_seen.find(as_integer(side))) {
                ...
                AdjacencyComponent adj_component(graph, move(component));
...
AdjacencyComponent::bipartition AdjacencyComponent::maximum_bipartite_partition_apx_1_2(uint64_t seed) {
    ...
    while (!no_optimizations_left && opt_steps < max_opt_steps) {
        ...
        for (size_t i = 0; i < component.size() && opt_steps < max_opt_steps; ++i) {
            ...
            if (partition.first.count(side)) {
                ...
                for_each_adjacent_side(side, [&]adj_side {
                    ...
                    if (edges_within + adj_edges_within - edges_across - adj_edges_across >= 1) {
                        ...
                        if (on_left) {
                            ...
                            partition.second.insert(adj_side);
                            partition.first.erase(adj_side);
                            partition.first.insert(side);
                            partition.second.erase(side);
                        }
...
uint64_t AdjacencyComponent::recursive_gray_code(bipartition& partition, uint64_t score, size_t index,
                                                 size_t to_flip, uint64_t& best_score, uint64_t& best_index) {
...
bipartition AdjacencyComponent::exhaustive_maximum_bipartite_partition() const {
    ...
    if (component.size() > 1) {
        ...
        recursive_gray_code(partition, 0, 1 << (component.size() - 2),
                            sides.size() - 2, best_score, best_index)
    }
...
void AdjacencyComponent::decompose_into_bipartite_blocks(const function<void(const HandleGraph&,const bipartition&)>& lambda) const {
    ...
    if (partition.first.size() + partition.second.size() == component.size()) {
        ...
        SubtractiveHandleGraph partition_graph(*super), remainder_graph(*super);
