...
void duplicate_terminus(
    ...
    // Only store the right side of the node, which will not be further modified by successive recursions
    ...
    // Otherwise duplicate and pass on the leftovers (LEFT-overs! ha!) to be split and duplicated again
    else {
