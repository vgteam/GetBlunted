...
void duplicate_prefix(
    ...
    if (size < graph.get_length(parent_node)) {
        fragments = graph.divide_handle(parent_node, size);
    }
    else if (size == graph.get_length(parent_node)) {
        fragments = {parent_node, parent_node};
    }
    else {
        throw runtime_error("ERROR: cannot duplicate overlap of length " + to_string(size) +
                            " which is longer than parent node (id): " + to_string(graph.get_id(parent_node)));
    }
...
void duplicate_suffix(
    ...
    if (size < graph.get_length(parent_node)) {
        size_t coordinate = graph.get_length(parent_node) - size;
        fragments = graph.divide_handle(parent_node, coordinate);
    }
    else if (size == graph.get_length(parent_node)) {
        fragments = {parent_node, parent_node};
    }
